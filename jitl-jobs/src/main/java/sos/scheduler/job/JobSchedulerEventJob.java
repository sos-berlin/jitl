...
public class JobSchedulerEventJob extends JobSchedulerJobAdapter {
    ...
    public boolean spooler_process() throws Exception {
        ...
        try {
            ...
            if (filter.getEventId() == null) {
                ...
                LOGGER.warn("error occurred processing event [" + filter.getEventClass() + " " + filter.getEventId() + "]: " + e.getMessage(), e);
            }
            return false;
        }
    }
    ...
    private void processSchedulerEvents() throws Exception {
        ...
        XmlEventHandler xmlEventHandler = new XmlEventHandler(eventHandlerResultedCommands, listOfEvents, eventHandlerFilepath, filter.getJobChain(),
                filter.getEventClass());
        xmlEventHandler.getListOfCommands();

        XsltEventHandler xsltEventHandler = new XsltEventHandler(eventHandlerResultedCommands, schedulerEventDBLayer.getEventsAsXml(filter
                .getSchedulerId(), this.listOfEvents), eventHandlerFilepath, this.eventHandlerFilespec, filter.getJobName(), filter.getJobChain(),
                filter.getEventClass(), filter.getExpirationDate());
        xsltEventHandler.getListOfCommands();

        EventCommandsExecuter commandsExecuter = new EventCommandsExecuter(eventHandlerResultedCommands, listOfEvents, spooler.hostname(),
                this.httpPort, this.socketTimeout);
