...
public class ManagedJobChainExport {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (!packageName.isEmpty() && !modelID.isEmpty()) {
                LOGGER.debug("jobchain und package dürfen nicht zusammen angegeben werden.");
                showUsage();
                System.exit(0);
    ...
    private static void export(String xmlFile, String modelID, String packageName) throws Exception {
        ...
        int settings = export.query(JobSchedulerManagedObject.getTableSettings(), "APPLICATION,SECTION,NAME", selSettings, "TYPE,TYPE,TYPE",
                jobTypes);
        int settingsOrders = export.query(JobSchedulerManagedObject.getTableSettings(), "APPLICATION,SECTION,NAME", selSettings, "TYPE,TYPE,TYPE",
                jobTypes2);
        export.doExport();
    }
