    ...
    public Subprocess executeSubprocess(final String command, final String commandParameters, final HashMap environment) throws Exception {
        ...
        try {
            ...
            for (String parameterName : parameterNames) {
                commandLine =
                        myReplaceAll(commandLine, "\\$\\{" + parameterName + "\\}",
        ...
        try {
            if (spooler_task.job().order_queue() != null) {
                ...
                while ((inBytesRead = in.read(inBuffer)) != -1) {
                    configurationBuilder.append(new String(inBuffer, 0, inBytesRead));
                }
                spooler_task.order().set_xml_payload(configurationBuilder.toString());
