        ...
        try {
            ...
            if (order.params().value(conParamNameCONFIGURATION_PATH) != null && !order.params().value(conParamNameCONFIGURATION_PATH).isEmpty()) {
                ...
                if (!spooler_job.configuration_directory().isEmpty()) {
                    ...
                    spooler_log.debug7("Looking for job chain configuration path: " + sJobChainPath.getAbsolutePath());
                    if (!sJobChainPath.exists()) {
                        spooler_log.debug2("Job Chain is probably configured in cache folder and not in live folder...");
                ...
                spooler_log.debug2(".. parameter [" + conParamNameCONFIGURATION_PATH + "]: " + this.getConfigurationPath());
            }
                ...
                if (spooler_job.order_queue() != null) {
                    if (!spooler_job.configuration_directory().isEmpty()) {
                        ...
                        if (confOrderFile.exists()) {
                            ...
                            spooler_log.debug2(".. configuration file for this order exists. order_id:" + order.job_chain().name() + "/" + order.id());
                        } else {
                            this.setConfigurationFilename(confFile.getAbsolutePath());
                            spooler_log.debug2(".. configuration file for job chain:" + order.job_chain().name() + "=" + this.getConfigurationFilename());
                        }
                    } else {
                        this.setConfigurationFilename("scheduler_" + spooler_task.order().job_chain().name() + conFileNameExtensionCONFIG_XML);
                    }
                    spooler_log.debug2(".. parameter [" + conParamNameCONFIGURATION_FILE + "]: " + this.getConfigurationFilename());
                }
            }
            File confFile = null;
