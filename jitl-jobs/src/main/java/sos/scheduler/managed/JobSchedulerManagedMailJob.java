...
public class JobSchedulerManagedMailJob extends JobSchedulerManagedJob {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(JobSchedulerManagedMailJob.class);
                ...
                try {
                    ...
                    if (this.getParameters().get("port") != null && !this.getParameters().get("port").isEmpty()) {
                        try {
                            ...
                            throw new Exception("illegal, non-numeric value [" + this.getParameters().get("port") + "] for parameter [port]: " + e
                                    .getMessage());
                        }
                    }
                    ...
                    if (this.getParameters().get("charset") != null && !this.getParameters().get("charset").isEmpty()) {
                    ...
                    if (this.getParameters().get("attachment_content_type") != null && !this.getParameters().get("attachment_content_type")
                            .isEmpty()) {
                    ...
                    if (this.getParameters().get("cleanup_attachment") != null && !this.getParameters().get("cleanup_attachment").isEmpty() && ("1"
                            .equals(this.getParameters().get("cleanup_attachment")) || "true".equalsIgnoreCase(this.getParameters().get(
                                    "cleanup_attachment")) || "yes".equalsIgnoreCase(this.getParameters().get("cleanup_attachment")))) {
                ...
                try {
                    ...
                    if (this.getConnectionSettings() != null) {
                        try {
                            ...
                            LOGGER.trace("No database settings found, using defaults from factory.ini");
                        }
                    }
                    if (mailSection != null) {
                        ...
                        try {
                            ...
                            if (!smtpProperties.isEmpty()) {
                                ...
                                if (smtpProperties.getProperty("mail.smtp.password") != null && !smtpProperties.getProperty("mail.smtp.password")
                                        .isEmpty()) {
