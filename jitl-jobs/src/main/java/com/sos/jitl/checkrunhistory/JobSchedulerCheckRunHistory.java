...
public class JobSchedulerCheckRunHistory extends JSToolBox implements JSJobUtilities, IJSCommands {
    ...
    private final static Logger LOGGER = Logger.getLogger(JobSchedulerCheckRunHistory.class);
    ...
    public JobSchedulerCheckRunHistory Execute() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < historyObjectNames.length; i++) {
                ...
                options().numberOfCompleted.Value(options().numberOfCompleted.Value() + delimiter + actHistoryObjectName + ":" + String.valueOf(jobHistory.getNumberOfCompleted()));
                options().numberOfCompletedSuccessful.Value(options().numberOfCompletedSuccessful.Value() + delimiter + actHistoryObjectName + ":" + String.valueOf(jobHistory.getNumberOfCompletedSuccessful()));
                options().numberOfCompletedWithError.Value(options().numberOfCompletedWithError.Value() + delimiter  + actHistoryObjectName + ":" + String.valueOf(jobHistory.getNumberOfCompletedWithError()));
                options().numberOfStarts.Value(options().numberOfStarts.Value() + delimiter + actHistoryObjectName + ":" + String.valueOf(jobHistory.getCount()));
