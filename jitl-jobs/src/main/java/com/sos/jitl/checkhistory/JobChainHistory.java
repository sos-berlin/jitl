...
public class JobChainHistory implements IJobSchedulerHistory {
	...
	public IJobSchedulerHistoryInfo getJobSchedulerHistoryInfo(String jobChainName, String timeLimit_)
			throws Exception {
		lastCompletedHistoryEntry = null;
		lastRunningHistoryEntry = null;
		lastCompletedSuccessfullHistoryEntry = null;
	...
	public IJobSchedulerHistoryInfo jobSchedulerHistoryInfo(String jobChainName) throws Exception {
		...
		if (lastCompletedHistoryEntry != null) {
			orderHistoryItem = historyWebserviceExecuter
					.getJobChainOrderHistoryEntry(lastCompletedHistoryEntry.getHistoryId());
		}
		...
		if (lastCompletedHistoryEntry != null) {
			...
			jobChainHistoryInfo.lastCompleted.top = (lastRunningHistoryEntry == null
					|| lastRunningHistoryEntry.getHistoryId() == null
					|| lastRunningHistoryEntry.getHistoryId().compareTo(lastCompletedHistoryEntry.getHistoryId()) <= 0);
			jobChainHistoryInfo.lastCompleted.start = jobHistoryHelper
					.getDateFromString(lastCompletedHistoryEntry.getStartTime());
			jobChainHistoryInfo.lastCompleted.end = jobHistoryHelper
					.getDateFromString(lastCompletedHistoryEntry.getEndTime());
			...
			jobChainHistoryInfo.lastCompleted.duration = jobHistoryHelper
					.getDuration(jobChainHistoryInfo.lastCompleted.start, jobChainHistoryInfo.lastCompleted.end);
		...
		if (lastCompletedSuccessfullHistoryEntry != null) {
			...
			jobChainHistoryInfo.lastCompletedSuccessful.top = (lastRunningHistoryEntry == null
					|| lastRunningHistoryEntry.getHistoryId() == null
					|| lastRunningHistoryEntry.getHistoryId()
							.compareTo(lastCompletedSuccessfullHistoryEntry.getHistoryId()) <= 0)
					&& (lastCompletedWithErrorHistoryEntry == null
							|| lastCompletedSuccessfullHistoryEntry.getHistoryId() == null
							|| lastCompletedWithErrorHistoryEntry.getHistoryId()
									.compareTo(lastCompletedSuccessfullHistoryEntry.getHistoryId()) <= 0);
			jobChainHistoryInfo.lastCompletedSuccessful.start = jobHistoryHelper
					.getDateFromString(lastCompletedSuccessfullHistoryEntry.getStartTime());
			jobChainHistoryInfo.lastCompletedSuccessful.end = jobHistoryHelper
					.getDateFromString(lastCompletedSuccessfullHistoryEntry.getEndTime());
			jobChainHistoryInfo.lastCompletedSuccessful.id = jobHistoryHelper
					.big2int(lastCompletedSuccessfullHistoryEntry.getHistoryId());
			jobChainHistoryInfo.lastCompletedSuccessful.jobChainName = lastCompletedSuccessfullHistoryEntry
					.getJobChain();
			jobChainHistoryInfo.lastCompletedSuccessful.duration = jobHistoryHelper.getDuration(
					jobChainHistoryInfo.lastCompletedSuccessful.start, jobChainHistoryInfo.lastCompletedSuccessful.end);
		} else {
		...
		if (lastCompletedWithErrorHistoryEntry != null) {
			...
			jobChainHistoryInfo.lastCompletedWithError.top = (lastRunningHistoryEntry == null
					|| lastRunningHistoryEntry.getHistoryId() == null
					|| lastRunningHistoryEntry.getHistoryId()
							.compareTo(lastCompletedWithErrorHistoryEntry.getHistoryId()) <= 0)
					&& (lastCompletedSuccessfullHistoryEntry == null
							|| lastCompletedSuccessfullHistoryEntry.getHistoryId() == null
							|| lastCompletedSuccessfullHistoryEntry.getHistoryId()
									.compareTo(lastCompletedWithErrorHistoryEntry.getHistoryId()) <= 0);
			jobChainHistoryInfo.lastCompletedWithError.start = jobHistoryHelper
					.getDateFromString(lastCompletedWithErrorHistoryEntry.getStartTime());
			jobChainHistoryInfo.lastCompletedWithError.end = jobHistoryHelper
					.getDateFromString(lastCompletedWithErrorHistoryEntry.getEndTime());
			jobChainHistoryInfo.lastCompletedWithError.id = jobHistoryHelper
					.big2int(lastCompletedWithErrorHistoryEntry.getHistoryId());
			jobChainHistoryInfo.lastCompletedWithError.jobChainName = lastCompletedWithErrorHistoryEntry.getJobChain();
			jobChainHistoryInfo.lastCompletedWithError.duration = jobHistoryHelper.getDuration(
					jobChainHistoryInfo.lastCompletedWithError.start, jobChainHistoryInfo.lastCompletedWithError.end);
		} else {
		...
		if (lastRunningHistoryEntry != null) {
			...
			jobChainHistoryInfo.running.start = jobHistoryHelper
					.getDateFromString(lastRunningHistoryEntry.getStartTime());
	...
	private void getHistoryByWebServiceCall(String jobChainName) throws Exception {
		...
		lastCompletedSuccessfullHistoryEntry = historyWebserviceExecuter
				.getLastCompletedSuccessfullJobChainHistoryEntry();
