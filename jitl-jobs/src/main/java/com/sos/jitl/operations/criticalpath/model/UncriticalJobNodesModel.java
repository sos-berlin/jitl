...
public class UncriticalJobNodesModel implements Serializable {
	...
	// processing mode: a value "-" will skip job nodes with that state prefix, a value "+" will skip job nodes without that state prefix
	...
	public void process() throws Exception{
		...
		try{
			...
			logger.info(String.format("%s: operation = %s (scheduler %s:%s)",
					method,
					operation.name().toLowerCase(),
					options.target_scheduler_host.Value(),
			...
			logger.info(String.format("%s: numberOfModifiedJobs = %s ",
					method,
	...
	/**
	 * Purpose: skip job nodes that are not required for critical path processing
     *          a) skip job nodes with negative state prefix
     *          b) skip job nodes without positive state prefix
	 *
	 * @throws Exception
	 */
	private void initProperties() throws Exception{
		...
		try{
			logger.info(String.format("%s: using parameter \"processing_recursive\" = %s",
					method,
					options.processing_recursive.Value()));
	
			
			if(!SOSString.isEmpty(options.processing_prefix.Value())){
				logger.info(String.format("%s: using parameter \"processing_prefix\" = %s",
						method,
				...
				if(options.processing_prefix.Value().startsWith(criticalPathNegativeProcessingPrefix)){
					...
					throw new Exception(String.format("%s: illegal value of parameter \"processing_prefix\" = %s",
							method,
							options.processing_prefix.Value()));
				}
			}
			...
			if(!SOSString.isEmpty(options.include_job_chains.Value())){
				logger.info(String.format("%s: using parameter \"include_job_chains\" = %s",
						method,
						options.include_job_chains.Value()));
				
				includePrefixedJobChains = options.include_job_chains.Value().split(";");
			}
	
			if(!SOSString.isEmpty(options.exclude_job_chains.Value())){
				logger.info(String.format("%s: using parameter \"exclude_job_chains\" = %s",
						method,
						options.exclude_job_chains.Value()));
	...
	private void execute(boolean skip) throws Exception{
        ...
        //command = "<show_state subsystems=\"order folder\" what=\"folders job_chains\"/>";
	    try{
        	connect();
			...
			for (int i = 0; i < jobChains.getLength(); ++i) {
				...
				if(!state.equalsIgnoreCase(EJobChainState.RUNNING.name())){
				   logger.info(String.format("%s: %s) do not process job chain [%s] due to state: %s",
						   method,
						   countJobChains,
						   state,
						   path));
				...
				if(includePrefixedJobChains != null &&  includePrefixedJobChains.length > 0){
					...
					for(String prefix : includePrefixedJobChains){
						logger.debug(String.format("%s: %s) path = %s include = %s",
								   method,
								   countJobChains,
								   path,
								   prefix));
					...
					if(doSkipJobChain){
						...
						logger.info(String.format("%s: %s) do not process job chain [%s] due to inclusion rule = %s",
								method,
								countJobChains,
								path,
				...
				if(doSkipJobChain == false && excludePrefixedJobChains != null){
					for(String prefix : excludePrefixedJobChains){
						logger.debug(String.format("%s: %s) path = %s exclude = %s",
								   method,
								   countJobChains,
								   path,
								   prefix));
						...
						if(path.indexOf(prefix) == 0){
							logger.info(String.format("%s: %s) do not process job chain [%s] due to exclusion rule = %s",
									method,
									countJobChains,
									path,
									prefix));
				...
				logger.info(String.format("%s: %s) processing job chain = %s",
						method,
						countJobChains,
						path));
				...
				for (int j = 0; j < jobChainNodes.getLength(); ++j) {
					...
					if(nodeJob.toLowerCase().startsWith("/scheduler_file_order")){
						logger.debug(String.format("%s: %s) %s continue processing, file order element found = %s",
								method,
								countJobChains,
								logIndent,
								nodeJob));
					...
					logger.debug(String.format("%s: %s) %s job node [%s] found with state = %s",
							method,
							countJobChains,
							logIndent,
							nodeJob,
					...
					String criticalPathProcessingPrefix = nodeState.substring(0,(criticalPathProcessingMode.equals("-") ? criticalPathNegativeProcessingPrefix.length() : criticalPathPositiveProcessingPrefix.length()));
					logger.debug(String.format("%s: %s) %s criticalPathProcessingPrefix=%s criticalPathProcessingMode=%s criticalPathNegativeProcessingPrefix=%s criticalPathPositiveProcessingPrefix=%s",
							 method,
							 countJobChains,
							 logIndent,
							 criticalPathProcessingPrefix,
							 criticalPathProcessingMode,
							 criticalPathNegativeProcessingPrefix,
							 criticalPathPositiveProcessingPrefix
							 ));   
					
					if(skip){
						// for negative processing skip nodes that are marked with "-"
	                    ...
	                    // for positive processing skip nodes that are not marked with "+"
	                    ...
	                    if(doSkip) {
	                        logger.info(String.format("%s: %s) %s skipping job node: job_chain='%s' state='%s'",
	                        		method,
	                        		countJobChains,
	                        		logIndent,
	                        		path,
	                        ...
	                        command = String.format("<job_chain_node.modify job_chain='%s' state='%s' action='next_state'/>",
	                        		path,
	                        		nodeState);
	                        executeXml(command);
	                       numberOfModifiedJobs++;
	                    	...
	                    	logger.debug(String.format("%s: %s) %s do not skipping job node: job_chain='%s' state='%s'",
	                        		method,
	                        		countJobChains,
	                        		logIndent,
	                        		path,
					   ...
					   // for negative processing unksip nodes that are marked with "-"
	                   if (criticalPathProcessingMode.equals(criticalPathNegativeProcessingPrefix) 
	                	&& criticalPathProcessingPrefix.equals(criticalPathNegativeProcessingPrefix)){
	                        doUnskip = true;
	                   }
	                    
	                    // for positive processing unskip nodes that are not marked with "+"
	                    if(criticalPathProcessingMode.equals(criticalPathPositiveProcessingPrefix.substring(0,1)) 
	                    	&& !criticalPathProcessingPrefix.equals(criticalPathPositiveProcessingPrefix)) {
	                        doUnskip = true;
	                    }

	                    // for positive processing unskip nodes that are marked with "+"
	                    if(criticalPathProcessingMode.equals(criticalPathPositiveProcessingPrefix.substring(0,1)) 
	                    	&& criticalPathProcessingPrefix.equals(criticalPathPositiveProcessingPrefix)){
	                        doUnskip = true;
	                    }
	                    ...
	                    if(doUnskip) {
	                    	  logger.info(String.format("%s: %s) %s unskipping job node: job_chain='%s' state='%s'",
		                        		method,
		                        		countJobChains,
		                        		logIndent,
		                        		path,
		                        ...
		                        command = String.format("<job_chain_node.modify job_chain='%s' state='%s' action='process'/>",
		                        		path,
		                        		nodeState); 
		                        executeXml(command);
		                        numberOfModifiedJobs++;
	                    	...
	                    	logger.debug(String.format("%s: %s) %s do not unskipping job node: job_chain='%s' state='%s'",
	                        		method,
	                        		countJobChains,
	                        		logIndent,
	                        		path,
        	...
        	throw new Exception(String.format("%s:[scheduler %s:%s] %s",
        			method,
        			options.target_scheduler_host.Value(),
        			options.target_scheduler_port.value(),
        			ex.toString()));
	...
	private void connect() throws Exception{
		...
		if(schedulerCommand != null){
			logger.info(String.format("%s: connect to scheduler %s:%s",
					method,
					options.target_scheduler_host.Value(),
					options.target_scheduler_port.value()));
	...
	private void disconnect(){
		...
		if(schedulerCommand != null){
			try{
				logger.info(String.format("%s: disconnect from scheduler %s:%s",
						method,
						options.target_scheduler_host.Value(),
						options.target_scheduler_port.value()));
