...
public class AggregationModel extends ReportingModel implements IReportingModel {
	...
	private DBItemReportExecutionDate insertReportingExecutionDate(EReferenceType type, 
		...
		DBItemReportExecutionDate item = createReportExecutionDate(
				schedulerId, 
				historyId, 
				type.value(), 
				id, 
			...
			while (rsTriggers.next()) {
				...
				try{
					...
					while(rsExecutions.next()){
						...
						DBItemReportExecutionDate exd = insertReportingExecutionDate(EReferenceType.EXECUTION, 
							execution.getSchedulerId(), 
							execution.getHistoryId(), 
							execution.getId(), 
				...
				DBItemReportExecutionDate exd = insertReportingExecutionDate(EReferenceType.TRIGGER, 
						trigger.getSchedulerId(), 
						trigger.getHistoryId(), 
						trigger.getId(), 
				...
				DBItemReportTriggerResult rtr = createReportTriggerResults(
						trigger.getSchedulerId(),
						trigger.getHistoryId(),
						trigger.getId(),
						startCause,
	...
	private void logSummary(DateTime start) throws Exception {
		...
		logger.info(String.format("%s: updated from inventory: triggers  = %s, executions  = %s",
				method,
