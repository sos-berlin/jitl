...
public class FactModel extends ReportingModel implements IReportingModel {
	...
	public FactModel(SOSHibernateSession reportingSess, SOSHibernateSession schedulerSess, FactJobOptions opt,
			JsonArray es) throws Exception {
		...
		taskHistoryMaxUncompletedAge = ReportUtil
				.resolveAge2Minutes(options.task_history_max_uncompleted_age.getValue());
		orderHistoryMaxUncompletedAge = ReportUtil
				.resolveAge2Minutes(options.order_history_max_uncompleted_age.getValue());
	...
	public void process() throws Exception {
		...
		if (isDebugEnabled) {
			LOGGER.debug(String.format("[%s]execute_notification_plugin=%s", method,
					options.execute_notification_plugin.value()));
		}
		...
		try {
			...
			if (isLocked) {
				LOGGER.info(String.format("[%s to %s UTC][skip synchronizing][locked]%s",
						reportingVariable.getTextValue(), dateToAsString, lockCause));
			} else {
				...
				if (doProcessing(dateFrom, dateTo)) {
					...
					synchronizeTimePeriodTasks(schedulerId, dateFrom, dateTo, dateToAsMinutes, dateFromAsString,
							dateToAsString);

					synchronizeUncompletedOrders(schedulerId, dateToAsMinutes);
					synchronizeTimePeriodOrders(schedulerId, dateFrom, dateTo, dateToAsMinutes, dateFromAsString,
							dateToAsString);
					...
					LOGGER.info(String.format(
							"[%s to %s UTC][skip synchronizing]time diff in seconds < wait interval(%s s)",
							dateFromAsString, dateToAsString, waitInterval));
				}
			}
		} catch (Exception e) {
			try {
				if (!finisched && reportingVariable != null && !SOSString.isEmpty(reportingVariable.getTextValue())) {
					...
					LOGGER.info(String.format("[%s][%s][%s]reset synchronizing on exception", method,
							reportingVariable.getTextValue(), oldDateFrom));
					finishSynchronizing(reportingVariable, oldDateFrom);
				} else {
					LOGGER.info(String.format("[%s][%s][%s][skip]reset synchronizing on exception", method,
							reportingVariable, finisched));
				}
			} catch (Throwable ee) {
				LOGGER.warn(String.format("error occured during reset synchronizing on exception: %s", method,
						ee.toString()));
			}
	...
	private DBItemReportVariable initSynchronizing(Date dateTo, Long dateToAsSeconds, String dateToAsString)
			throws Exception {
		...
		try {
			...
			if (variable == null) {
				...
				variable = getDbLayer().insertReportVariable(name, new Long(maxHistoryAge),
						getSetLocked(dateFrom, dateToAsString));
				if (isDebugEnabled) {
					LOGGER.debug(String.format("[%s]dateFrom=%s (initial, %s-%s)", method,
							ReportUtil.getDateAsString(dateFrom), dateToAsString, maxHistoryAge));
				}
			} else {
				String dateFromAsString = variable.getTextValue();
				if (isDebugEnabled) {
					LOGGER.debug(String.format(
							"[%s]storedDateFrom=%s, dateTo=%s, storedMaxAge=%s, storedLockVersion=%s", method,
							dateFromAsString, dateToAsString, variable.getNumericValue(), variable.getLockVersion()));
				}
				Date dateFrom = null;
				if (SOSString.isEmpty(dateFromAsString)) {
					...
					if (dateFromAsString.startsWith(LOCK_PREFIX + LOCK_DELIMITER)) {
						...
						if (diff >= MAX_LOCK_WAIT) {
							...
							if (isDebugEnabled) {
								LOGGER.debug(String.format(
										"[%s][%s]set isLocked: diff=%ss(dateToAsSeconds=%s-anotherDateToAsSeconds=%s) >= MAX_LOCK_WAIT=%ss",
										method, dateFromAsString, diff, dateToAsSeconds, anotherDateToAsSeconds,
										MAX_LOCK_WAIT));
							}
			...
			getDbLayer().getSession().commit();
			if (isDebugEnabled) {
				LOGGER.debug(String.format("[%s]dateFrom=%s, maxAge=%s, lockVersion=%s", method,
						variable.getTextValue(), variable.getNumericValue(), variable.getLockVersion()));
			}
		} catch (SOSHibernateObjectOperationStaleStateException e) {
			try {
				getDbLayer().getSession().rollback();
			...
			if (isDebugEnabled) {
				LOGGER.debug(String.format("[%s]set isLocked on SOSHibernateObjectOperationStaleStateException: %s",
						method, e.toString()));
			}
			...
			if (triggers != null) {
				...
				if (size > 0) {
					...
					for (int i = 0; i < size; i++) {
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][%s][%s][step>=%s]%s", method, (i + 1),
									trigger.getParentName(), trigger.getName(), trigger.getResultSteps(),
									SOSString.toString(trigger)));
						}

						Query<DBItemSchedulerHistoryOrderStepReporting> query = getDbLayer()
								.getSchedulerHistoryOrderStepsQuery(schedulerSession, largeResultFetchSizeScheduler,
										schedulerId, trigger.getHistoryId(), trigger.getResultSteps());
						CounterSynchronize counter = synchronizeOrderHistory(method, query, OrderSync.UNCOMPLETED,
								dateToAsMinutes);
	...
	private List<Long> getTaskSyncUncomplitedHistoryIds(String schedulerId) throws Exception {
		...
		try {
			getDbLayer().getSession().beginTransaction();
			List<Long> result = getDbLayer().getTaskSyncUncomplitedHistoryIds(largeResultFetchSizeReporting,
					schedulerId);
			getDbLayer().getSession().commit();
			...
			if (size > SOSHibernate.LIMIT_IN_CLAUSE) {
				...
				for (int i = 0; i < size; i += SOSHibernate.LIMIT_IN_CLAUSE) {
					...
					if (i < size) {
						...
						Query<DBItemSchedulerHistory> query = getDbLayer().getSchedulerHistoryTasksQuery(
								schedulerSession, largeResultFetchSizeScheduler, schedulerId, subList);
						CounterSynchronize counter = synchronizeTaskHistory(method, query, TaskSync.UNCOMPLETED,
								schedulerId, dateToAsMinutes);
						counterTotal += counter.getTotal();
						counterSkip += counter.getSkip();
						counterInsertedTriggers += counter.getInsertedTriggers();
						counterUpdatedTriggers += counter.getUpdatedTriggers();
				...
				Query<DBItemSchedulerHistory> query = getDbLayer().getSchedulerHistoryTasksQuery(schedulerSession,
						largeResultFetchSizeScheduler, schedulerId, uncompletedTaskHistoryIds);
				counterTaskSyncUncompleted = synchronizeTaskHistory(method, query, TaskSync.UNCOMPLETED, schedulerId,
						dateToAsMinutes);
			}
		}
	}

	private void synchronizeTimePeriodOrders(String schedulerId, Date dateFrom, Date dateTo, Long dateToAsMinutes,
			String dateFromAsString, String dateToAsString) throws Exception {
		String method = "synchronizeTimePeriodOrders";
		if (isDebugEnabled) {
			LOGGER.debug(String.format("[%s][%s to %s UTC]", method, dateFromAsString, dateToAsString));
		}
		Query<DBItemSchedulerHistoryOrderStepReporting> query = getDbLayer().getSchedulerHistoryOrderStepsQuery(
				schedulerSession, largeResultFetchSizeScheduler, schedulerId, dateFrom, dateTo);
		counterOrderSync = synchronizeOrderHistory(method, query, OrderSync.PERIOD, dateToAsMinutes);
	}

	private void synchronizeTimePeriodTasks(String schedulerId, Date dateFrom, Date dateTo, Long dateToAsMinutes,
			String dateFromAsString, String dateToAsString) throws Exception {
	...
	private void analyzeEvents(JsonArray events) throws Exception {
		...
		try {
				...
				if (joType.equals(EventType.TaskClosed.name())) {
					...
					if (key != null && key.getValueType().equals(ValueType.OBJECT)) {
						try {
							...
							LOGGER.error(String.format("[%s]can't get taskId from TaskClosed event: %s", method,
									e.toString()), e);
						}
					}
			...
			LOGGER.error(String.format("[%s]exception on get taskId from TaskClosed/TaskStarted events: %s", method,
					e.toString()), e);
		}

		if (isDebugEnabled) {
			LOGGER.debug(String.format("[%s]TaskClosed=%s, TaskStarted=%s", method, eventTaskClosedHistoryIds.size(),
					eventTaskStartedHistoryIds.size()));
		}
	}
	...
	private void synchronizeTaskStartedEvents(String schedulerId, Long dateToAsMinutes) throws Exception {
		...
		if (size > 0) {
			...
			try {
				getDbLayer().getSession().beginTransaction();
				List<Long> dbHistoryIds = getDbLayer().getTasksHistoryIds(largeResultFetchSizeReporting, schedulerId,
						eventHistoryIds);
				for (int i = 0; i < eventHistoryIds.size(); i++) {
					...
					if (!dbHistoryIds.contains(historyId)) {
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s]task.historyId=%s", method, ts.getJobPath(),
									ts.getHistoryId()));
						}

						List<Map<String, String>> infos = null;
						try {
							infos = getDbLayer().getInventoryJobInfoByJobName(schedulerId,
									options.current_scheduler_hostname.getValue(),
									options.current_scheduler_http_port.value(), ts.getJobPath());
						} catch (Exception e) {
							throw new Exception(
									String.format("error on getInventoryJobInfoByJobName: %s", e.toString()), e);
						}
						InventoryInfo inventoryInfo = getInventoryInfo(method, infos);
						if (isDebugEnabled) {
							LOGGER.debug(String.format(
									"[%s][%s][inventory_info]title=%s, isRuntimeDefined=%s, isOrderJob=%s", method,
									ts.getJobPath(), inventoryInfo.getTitle(), inventoryInfo.getIsRuntimeDefined(),
									inventoryInfo.getIsOrderJob()));
						}
	...
	private void synchronizeTaskClosedEvents(String schedulerId, Long dateToAsMinutes) throws Exception {
		...
		if (size > 0) {
			if (size > SOSHibernate.LIMIT_IN_CLAUSE) {
				LOGGER.info(String.format("[%s]%s TaskClosed events > as %s. do split...", method, size,
						SOSHibernate.LIMIT_IN_CLAUSE));
				int counterTotal = 0;
				int counterSkip = 0;
				int counterInsertedTriggers = 0;
				...
				for (int i = 0; i < size; i += SOSHibernate.LIMIT_IN_CLAUSE) {
					...
					CounterSynchronize counter = synchronizeTaskHistory(method, query, TaskSync.EVENT, schedulerId,
							dateToAsMinutes);
					counterTotal += counter.getTotal();
					counterSkip += counter.getSkip();
					counterInsertedTriggers += counter.getInsertedTriggers();
				...
				Query<DBItemSchedulerHistory> query = getDbLayer().getSchedulerHistoryTasksQuery(schedulerSession,
						largeResultFetchSizeScheduler, schedulerId, eventTaskClosedHistoryIds);
				counterTaskSyncEvent = synchronizeTaskHistory(method, query, TaskSync.EVENT, schedulerId,
						dateToAsMinutes);
			}
		}
	}

	private boolean calculateIsSyncCompleted(int maxUncompletedAge, Date startTime, Date endTime,
			Long dateToAsMinutes) {
	...
	private synchronized CounterSynchronize synchronizeTaskHistory(String caller, Query<DBItemSchedulerHistory> query,
			TaskSync taskSync, String schedulerId, Long dateToAsMinutes) throws Exception {
	...
	private void handleException(String callerMethod, Exception e, int count) throws Exception {
		if (count >= MAX_RERUNS) {
			...
			if (lae == null) {
				...
				LOGGER.warn(String.format("[%s]%s occured, wait %ss and try again (%s of %s) ...", callerMethod,
						lae.getClass().getName(), RERUN_INTERVAL, count, MAX_RERUNS));
				Thread.sleep(RERUN_INTERVAL * 1000);
			}
		}
	}

	private List<DBItemSchedulerHistory> getSchedulerHistoryTasks(Query<DBItemSchedulerHistory> query)
			throws Exception {
	...
	private CounterSynchronize synchronizeTaskHistory(String caller, List<DBItemSchedulerHistory> result,
			TaskSync taskSync, String schedulerId, Long dateToAsMinutes) throws Exception {
			...
			if (totalSize > 0) {
				...
				for (int i = 0; i < totalSize; i++) {
					...
					if (task.getJobName().equals("(Spooler)")) {
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][skip][scheduler][task][%s]%s", method, counterTotal,
					...
					if (isDebugEnabled) {
						LOGGER.debug(String.format("[%s][%s][scheduler][task][%s][%s]%s", method, counterTotal,
								task.getId(), task.getJobName(), SOSString.toString(task)));
					}
					...
					if (taskSync.equals(TaskSync.PERIOD)) {
						if (eventTaskClosedHistoryIds.contains(task.getId())) {
							if (isDebugEnabled) {
								LOGGER.debug(String.format(
										"[%s][%s][skip][scheduler][task][%s][%s]was already handled by event TaskClose",
										method, counterTotal, task.getId(), task.getJobName()));
							}
							counterSkip++;
							continue;
						}
					...
					boolean syncCompleted = calculateIsSyncCompleted(taskHistoryMaxUncompletedAge, task.getStartTime(),
							task.getEndTime(), dateToAsMinutes);
					...
					if (reportTask == null) {
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][inserted][%s][%s][%s]%s", method, counterTotal,
									reportTask.getId(), reportTask.getHistoryId(), reportTask.getName(),
									SOSString.toString(reportTask)));
						}
						counterInserted++;
					} else {
						boolean updateExecutions = false;
						if (reportTask.getError() && (reportTask.getExitCode() == null
								|| reportTask.getExitCode().equals(new Integer(0)))) {
							updateExecutions = true;
						}
						getDbLayer().updateTask(reportTask, task, syncCompleted);
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][updated][%s][%s][%s]%s", method, counterTotal,
									reportTask.getId(), reportTask.getHistoryId(), reportTask.getName(),
									SOSString.toString(reportTask)));
						}
						if (updateExecutions) {
							...
							if (isDebugEnabled) {
								LOGGER.debug(String.format(
										"[%s][%s][updateComplitedExecutionsByTask][%s][%s][%s]updated executions=%s",
										method, counterTotal, reportTask.getId(), reportTask.getHistoryId(),
										reportTask.getName(), rc));
							}
						}
					...
					if (counterTotal % options.log_info_step.value() == 0) {
						LOGGER.info(String.format("[%s]%s of %s history steps processed ...", method, counterTotal,
								totalSize));
					}
				}
				getDbLayer().getSession().commit();
				if (isDebugEnabled) {
					LOGGER.debug(
							String.format("[%s]total history tasks=%s, inserted=%s, updated=%s, skip=%s, duration=%s",
									method, counterTotal, counterInserted, counterUpdated, counterSkip,
	...
	private synchronized CounterSynchronize synchronizeOrderHistory(String caller,
			Query<DBItemSchedulerHistoryOrderStepReporting> query, OrderSync orderSync, Long dateToAsMinutes)
			throws Exception {
	...
	private List<DBItemSchedulerHistoryOrderStepReporting> getSchedulerHistoryOrderSteps(
			Query<DBItemSchedulerHistoryOrderStepReporting> query) throws Exception {
	...
	private CounterSynchronize synchronizeOrderHistory(String caller,
			List<DBItemSchedulerHistoryOrderStepReporting> result, OrderSync orderSync, Long dateToAsMinutes)
			throws Exception {
		...
		try {
			...
			if (totalSize > 0) {
				...
				for (int i = 0; i < totalSize; i++) {
					...
					if (step.getOrderHistoryId() == null && step.getOrderId() == null
							&& step.getOrderStartTime() == null) {
						counterSkip++;
						if (isDebugEnabled) {
							LOGGER.debug(String.format(
									"[%s][%s][scheduler][orderStep][skip][order object is null]step=%s, historyId=%s ",
									method, counterTotal, step.getStepState(), step.getStepHistoryId()));
						}
						continue;
					}
					if (isDebugEnabled) {
						LOGGER.debug(String.format("[%s][%s][scheduler][orderStep][%s][%s][%s]%s", method, counterTotal,
								step.getOrderJobChain(), step.getOrderId(), step.getStepStep(),
								SOSString.toString(step)));
					}
					...
					if (step.getTaskId() == null && step.getTaskJobName() == null && step.getTaskCause() == null) {
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][scheduler][orderStep][%s][%s][%s]task object is null",
									method, counterTotal, step.getOrderJobChain(), step.getOrderId(),
									step.getStepStep()));
						}
						reportTask = getDbLayer().getTask(step.getOrderSchedulerId(), step.getStepTaskId());
						if (reportTask == null
								|| reportTask.getBasename().equals(DBLayerReporting.NOT_FOUNDED_JOB_BASENAME)) {
							List<Map<String, String>> infos = null;
							try {
								infos = getDbLayer().getInventoryJobInfoByJobChain(schedulerId,
										options.current_scheduler_hostname.getValue(),
										options.current_scheduler_http_port.value(), step.getOrderJobChain(),
										step.getStepState());
							} catch (Exception e) {
								throw new Exception(
										String.format("error on getInventoryJobInfoByJobChain: %s", e.toString()), e);
							}
							...
							if (reportTask == null) {
								try {
									...
									if (isDebugEnabled) {
										LOGGER.debug(String.format(
												"[%s][%s][task][%s][%s][%s][inserted by order step]%s", method,
												counterTotal, reportTask.getId(), reportTask.getHistoryId(),
												reportTask.getName(), SOSString.toString(reportTask)));
									}
								} catch (Exception e) {
								...
								if (isDebugEnabled) {
									LOGGER.debug(String.format(
											"[%s][%s][task][%s][%s][%s]already exists.  try to find the correct name ...",
											method, counterTotal, reportTask.getId(), reportTask.getHistoryId(),
											reportTask.getName()));
								}
								if (taskInventoryInfo != null && taskInventoryInfo.getName() != null) {
									...
									if (isDebugEnabled) {
										LOGGER.debug(String.format(
												"[%s][%s][task][%s][%s][%s][updated from inventory]%s", method,
												counterTotal, reportTask.getId(), reportTask.getHistoryId(),
												reportTask.getName(), SOSString.toString(reportTask)));
									}
								}
							...
							if (isDebugEnabled) {
								LOGGER.debug(String.format("[%s][%s][task][%s][%s][%s][already exist]%s", method,
										counterTotal, reportTask.getId(), reportTask.getHistoryId(),
										reportTask.getName(), SOSString.toString(reportTask)));
							}
						}
						...
						checkReduceEventTaskStartedHistoryIds(method, counterTotal, reportTask.getHistoryId(),
								reportTask.getName());
					}
					...
					if (triggerObjects.containsKey(step.getOrderHistoryId())) {
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][use trigger][%s][%s]", method, counterTotal,
									rt.getId(), rt.getName()));
						}
					} else {
						boolean syncCompleted = calculateIsSyncCompleted(orderHistoryMaxUncompletedAge,
								step.getOrderStartTime(), step.getOrderEndTime(), dateToAsMinutes);
						rt = getDbLayer().getTrigger(step.getOrderSchedulerId(), step.getOrderHistoryId());
						if (rt == null) {
							List<Map<String, String>> infos = null;
							try {
								infos = getDbLayer().getInventoryOrderInfoByJobChain(step.getOrderSchedulerId(),
										options.current_scheduler_hostname.getValue(),
										options.current_scheduler_http_port.value(), step.getOrderId(),
										step.getOrderJobChain());
							} catch (Exception e) {
								throw new Exception(
										String.format("error on getInventoryOrderInfoByJobChain: %s", e.toString()), e);
							}
							...
							if (startCause.equals(EStartCauses.ORDER.value())) {
								String inventoryStartCause = getDbLayer().getInventoryJobChainStartCause(
										step.getOrderSchedulerId(), this.options.current_scheduler_hostname.getValue(),
										this.options.current_scheduler_http_port.value(),
							...
							if (isDebugEnabled) {
								LOGGER.debug(String.format("[%s][%s][trigger][inserted]%s", method, counterTotal,
										SOSString.toString(rt)));
							}
							...
							if (rt.getSyncCompleted() != syncCompleted || step.getOrderEndTime() != null) {
								...
								if (isDebugEnabled) {
									LOGGER.debug(String.format("[%s][%s][trigger][updated]%s", method, counterTotal,
											SOSString.toString(rt)));
								}
					...
					if (step.getStepStep() != null && rt.getResultSteps() != null) {
						if (step.getStepStep() < rt.getResultSteps()) {
							counterSkip++;
							if (isDebugEnabled) {
								LOGGER.debug(String.format("[%s][%s][skip][trigger][step %s < %s]%s", method,
										counterTotal, step.getStepStep(), rt.getResultSteps(), SOSString.toString(rt)));
							}
							continue;
						} else if (step.getStepStep() == rt.getResultSteps()) {
							re = getDbLayer().getExecution(step.getOrderSchedulerId(), step.getStepTaskId(), rt.getId(),
									step.getStepStep());
							if (re != null) {
									...
									if (endedOrderTasks4notification.containsKey(re.getHistoryId())) {
										...
										if (isDebugEnabled) {
											LOGGER.debug(String.format(
													...
													method, counterTotal, re.getId(), step.getStepStep(),
													rt.getResultSteps(), SOSString.toString(re)));
										}
									} else {
										counterSkip++;
										if (isDebugEnabled) {
											LOGGER.debug(String.format(
													"[%s][%s][skip][execution][%s][syncCompleted=true][step %s=%s]%s",
													method, counterTotal, re.getId(), step.getStepStep(),
													rt.getResultSteps(), SOSString.toString(re)));
										}
										if (orderSync.equals(OrderSync.UNCOMPLETED)
												&& uncompletedTaskHistoryIds.contains(step.getStepTaskId())) {
											uncompletedTaskHistoryIds.remove(step.getStepTaskId());
											if (isDebugEnabled) {
												LOGGER.debug(String.format(
														"[%s][%s][task][%s][not founded]removed from memory. notFounded size=%s",
														method, counterTotal, step.getStepTaskId(),
														uncompletedTaskHistoryIds.size()));
											}
										}
								...
								boolean syncCompleted = calculateIsSyncCompleted(taskHistoryMaxUncompletedAge,
										step.getStepStartTime(), step.getStepEndTime(), dateToAsMinutes);
								boolean agentChanged = re.getAgentUrl() == null && step.getTaskAgentUrl() != null;
								boolean cmChanged = re.getClusterMemberId() == null
										&& step.getTaskClusterMemberId() != null;
								...
								if (isDebugEnabled) {
									LOGGER.debug(String.format(
											...
											method, counterTotal, re.getId(), step.getStepStep(), rt.getResultSteps(),
											syncCompleted, re.getName(), re.normalizePath(step.getTaskJobName()),
											re.getError(), step.isStepError(), re.getExitCode(), sec, agentChanged,
											cmChanged));
								}
								if (!syncCompleted && (re.getName().equals(re.normalizePath(step.getTaskJobName())))
										&& (re.getError() == step.isStepError()) && (sec.equals(re.getExitCode()))
										&& !agentChanged && !cmChanged) {
									counterSkip++;
									if (isDebugEnabled) {
										LOGGER.debug(String.format("[%s][%s][skip][execution][%s][not changed]%s",
												method, counterTotal, re.getId(), SOSString.toString(re)));
									}
					...
					if (re == null) {
						if (reportTask == null) {
							...
							if (reportTask == null) {// e.g. task created by splitter
								...
								try {
									infos = getDbLayer().getInventoryJobInfoByJobName(schedulerId,
											options.current_scheduler_hostname.getValue(),
											options.current_scheduler_http_port.value(),
											ReportUtil.normalizeDbItemPath(step.getTaskJobName()));
								} catch (Exception e) {
									throw new Exception(
											String.format("error on getInventoryJobInfoByJobName: %s", e.toString()),
											e);
								}
								InventoryInfo inventoryInfo = getInventoryInfo(method, infos);
								boolean syncCompleted = calculateIsSyncCompleted(taskHistoryMaxUncompletedAge,
										step.getTaskStartTime(), step.getTaskEndTime(), dateToAsMinutes);
								try {
								...
								if (isDebugEnabled) {
									LOGGER.debug(String.format("[%s][%s][reportTask][inserted]%s", method, counterTotal,
											SOSString.toString(reportTask)));
								}
								counterInsertedTasks++;
							}
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][execution][%s][inserted]%s", method, counterTotal,
									re.getId(), SOSString.toString(re)));
						}
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][execution][%s][updated]%s", method, counterTotal,
									re.getId(), SOSString.toString(re)));
						}
					...
					if (isDebugEnabled) {
						LOGGER.debug(String.format("[%s][%s][trigger][%s]step=%s, resultSteps=%s", method, counterTotal,
								rt.getId(), step.getStepStep(), rt.getResultSteps()));
					}
					if (step.getStepStep() >= rt.getResultSteps()) {
						try {
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][trigger][%s][result updated]%s", method, counterTotal,
									rt.getId(), SOSString.toString(rt)));
						}
						triggerObjects.put(step.getOrderHistoryId(), rt);
						counterUpdatedTriggers++;
					}
					...
					if (rt.getSyncCompleted() && reportTask != null
							&& uncompletedTaskHistoryIds.contains(reportTask.getHistoryId())) {
						if (reportTask.getEndTime() == null && !reportTask.getSyncCompleted()) {
							if (isDebugEnabled) {
								LOGGER.debug(String.format(
										"[%s][%s][task][%s][%s][not founded]remove from memory und set syncCompleted=1. notFounded size=%s",
										method, counterTotal, reportTask.getId(), reportTask.getHistoryId(),
										uncompletedTaskHistoryIds.size()));
							}
				...
				if (isDebugEnabled) {
					LOGGER.debug(
							String.format("[%s]duration=%s", method, ReportUtil.getDuration(start, new DateTime())));
				}
			}
			counter.setTotal(counterTotal);
			counter.setSkip(counterSkip);
	...
	private void checkReduceEventTaskStartedHistoryIds(String caller, int counter, Long taskHistoryId, String jobName) {
		if (eventTaskStartedHistoryIds.containsKey(taskHistoryId)) {
			...
			if (isDebugEnabled) {
				LOGGER.debug(String.format("[%s][%s][scheduler][task][%s][%s]removed from memory. TaskStarted size=%s",
						caller, counter, taskHistoryId, jobName, eventTaskStartedHistoryIds.size()));
			}
		}
	}
		...
		if (size > 0) {
			try {
				...
				for (Long historyId : uncompletedTaskHistoryIds) {
					...
					if (reportTask == null) {
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s]not found reportTasks for schedulerId=%s, historyId=%s",
									method, schedulerId, historyId));
						}
					} else {
						checkReduceEventTaskStartedHistoryIds(method, counterTotal, reportTask.getHistoryId(),
								reportTask.getName());
						...
						if (isDebugEnabled) {
							LOGGER.debug(String.format("[%s][%s][%s]%s", method, reportTask.getId(),
									reportTask.getName(), SOSString.toString(reportTask)));
						}
						if (reportTask.getIsOrder()) {
							List<DBItemReportExecution> executions = getDbLayer()
									.getExecutionsByTask(reportTask.getId());
							if (executions == null || executions.size() == 0) {
								if (isDebugEnabled) {
									LOGGER.debug(String.format("[%s][%s][%s]not found executions", method,
											reportTask.getId(), reportTask.getName()));
								}
							} else {
								if (isDebugEnabled) {
									LOGGER.debug(String.format("[%s][%s][%s]found %s executions", method,
											reportTask.getId(), reportTask.getName(), executions.size()));
								}
								...
								if (reportTask.getBasename().equals(DBLayerReporting.NOT_FOUNDED_JOB_BASENAME)) {
									...
									try {
										infos = getDbLayer().getInventoryJobInfoByJobChain(schedulerId,
												options.current_scheduler_hostname.getValue(),
												options.current_scheduler_http_port.value(), firstExecution.getFolder(),
												firstExecution.getState());
									} catch (Exception e) {
										throw new Exception(String.format("error on getInventoryJobInfoByJobChain: %s",
												e.toString()), e);
									}
									InventoryInfo taskInventoryInfo = getInventoryInfo(method, infos);
									if (isDebugEnabled) {
										LOGGER.debug(String.format("[%s][%s][%s]%s", method, reportTask.getId(),
												reportTask.getName(), SOSString.toString(taskInventoryInfo)));
									}
									if (taskInventoryInfo != null && taskInventoryInfo.getName() != null) {
										reportTask.setFolder(ReportUtil.getFolderFromName(taskInventoryInfo.getName()));
										reportTask.setName(taskInventoryInfo.getName());
										reportTask.setBasename(
												ReportUtil.getBasenameFromName(taskInventoryInfo.getName()));
										reportTask.setTitle(taskInventoryInfo.getTitle());
										doUpdate = true;
									}
								}
								...
								for (DBItemReportExecution execution : executions) {
									if (isDebugEnabled) {
										LOGGER.debug(String.format("[%s][%s][%s]%s", method, reportTask.getId(),
												reportTask.getName(), SOSString.toString(execution)));
									}
									...
									if (execution.getBasename().equals(DBLayerReporting.NOT_FOUNDED_JOB_BASENAME)
											&& doUpdate) {
								...
								if (syncCompleted && calculateIsSyncCompleted(taskHistoryMaxUncompletedAge,
										reportTask.getStartTime(), null, dateToAsMinutes)) {
									reportTask.setSyncCompleted(true);
									doUpdate = true;
								}
								if (lastExecutionWithEndTime != null) {
									DBItemReportTrigger trigger = getDbLayer()
											.getTrigger(lastExecutionWithEndTime.getTriggerId());

									if (isDebugEnabled) {
										LOGGER.debug(String.format("[%s][%s][%s][last execution with endTime]%s",
												method, reportTask.getId(), reportTask.getName(),
												SOSString.toString(lastExecutionWithEndTime)));
										LOGGER.debug(String.format(
												"[%s][%s][%s][last execution with endTime trigger]%s", method,
												reportTask.getId(), reportTask.getName(), SOSString.toString(trigger)));
									}
									if (trigger != null && trigger.getSyncCompleted()) {
										...
										LOGGER.debug(String.format(
												...
												method, reportTask.getId(), reportTask.getName()));
									} else if (calculateIsSyncCompleted(taskHistoryMaxUncompletedAge,
											lastExecutionWithEndTime.getStartTime(), null, dateToAsMinutes)) {
										reportTask.setEndTime(lastExecutionWithEndTime.getEndTime());
										reportTask.setSyncCompleted(true);
										LOGGER.debug(String.format(
							...
							counterUpdatedTasks++;
							if (isDebugEnabled) {
								LOGGER.debug(String.format("[%s][isOrder=0][set syncCompleted=1]%s", method,
										SOSString.toString(reportTask)));
							}
						}
					}
		...
		try {
			...
			for (DBItemReportTask task : endedOrderTasks4notification.values()) {
				...
				if (executions == null || executions.size() == 0) {
					if (isDebugEnabled) {
						LOGGER.debug(
								String.format("[%s][skip]not found executions for taskId=%s", method, task.getId()));
					}
					continue;
				}
				for (DBItemReportExecution execution : executions) {
					...
					if (row.size() > 3) {
						...
						if (row.size() > 4) {
							if (row.containsKey("cluster_type")) {
								item.setClusterType(
										SOSString.isEmpty(row.get("cluster_type")) ? null : row.get("cluster_type"));
							}
							if (row.containsKey("url")) {
								item.setUrl(SOSString.isEmpty(row.get("url")) ? null : row.get("url"));
							}
							if (row.containsKey("ordering")) {
								item.setOrdering(SOSString.isEmpty(row.get("ordering")) ? null
										: Integer.parseInt(row.get("ordering")));
							}
	...
	private void setChangedSummary() throws Exception {
		...
		if (counterOrderSync.getInsertedTriggers() > 0 || counterOrderSync.getUpdatedTriggers() > 0
				|| counterOrderSync.getInsertedExecutions() > 0 || counterOrderSync.getUpdatedExecutions() > 0
				|| counterOrderSyncUncompleted.getInsertedTriggers() > 0
				|| counterOrderSyncUncompleted.getUpdatedTriggers() > 0
				|| counterOrderSyncUncompleted.getInsertedExecutions() > 0
				|| counterOrderSyncUncompleted.getUpdatedExecutions() > 0
				|| counterTaskSyncNotFounded.getUpdatedExecutions() > 0) {
			isOrdersChanged = true;
			isChanged = true;
		}
		if (counterOrderSync.getInsertedTasks() > 0 || counterOrderSyncUncompleted.getInsertedTasks() > 0
				|| counterTaskSync.getInsertedTasks() > 0 || counterTaskSync.getUpdatedTasks() > 0
				|| counterTaskSyncUncompleted.getInsertedTasks() > 0 || counterTaskSyncUncompleted.getUpdatedTasks() > 0
				|| counterTaskSyncNotFounded.getUpdatedTasks() > 0 || counterTaskSyncEvent.getInsertedTasks() > 0
		...
		if (isChanged) {
			...
			if (isTasksChanged) {
				LOGGER.info(String.format(
						...
						from, to, range, counterTaskSync.getTotal(), counterTaskSync.getInsertedTasks(),
						counterTaskSync.getUpdatedTasks(), counterTaskSync.getSkip(),
						counterTaskSyncUncompleted.getTotal(), counterTaskSyncUncompleted.getInsertedTasks(),
						counterTaskSyncUncompleted.getUpdatedTasks(), counterTaskSyncUncompleted.getSkip(),
						counterTaskSyncEvent.getTotal(), counterTaskSyncEvent.getInsertedTasks(),
						counterTaskSyncEvent.getUpdatedTasks(), counterTaskSyncEvent.getSkip(),
						counterTaskStartedEventsInserted, counterTaskSyncNotFounded.getTotal(),
			...
			if (isOrdersChanged) {
				LOGGER.info(String.format(
						...
						from, to, range, counterOrderSync.getTotal(), counterOrderSync.getInsertedTriggers(),
						counterOrderSync.getUpdatedTriggers(), counterOrderSync.getInsertedExecutions(),
						counterOrderSync.getUpdatedExecutions(), counterOrderSync.getInsertedTasks(),
						counterOrderSync.getSkip(), counterOrderSyncUncompleted.getTotal(),
						counterOrderSyncUncompleted.getInsertedTriggers(),
						counterOrderSyncUncompleted.getUpdatedTriggers(),
						counterOrderSyncUncompleted.getInsertedExecutions(),
						counterOrderSyncUncompleted.getUpdatedExecutions(),
						counterOrderSyncUncompleted.getInsertedTasks(), counterOrderSyncUncompleted.getSkip()));
			} else {
				LOGGER.info(String.format("[%s to %s UTC][%s] 0 changes", from, to, range));
			}
	...
	private Date getDateFrom(DBItemReportVariable reportingVariable, Date dateTo, String dateToAsString)
			throws Exception {
		...
		if (options.force_max_history_age.value()) {
			...
			if (diffMinutes > currentMaxAge) {
				Long countHistoryTasks = getDbLayer().getCountSchedulerHistoryTasks(schedulerSession, schedulerId,
						storedDateFrom);
				if (countHistoryTasks > maxHistoryTasks) {
					dateFrom = null;
					LOGGER.info(String.format(
							"[%s][set dateFrom=null]diffMinutes=%s > currentMaxAge=%sm and countHistoryTasks=%s > maxHistoryTasks=%s",
							method, diffMinutes, currentMaxAge, countHistoryTasks, maxHistoryTasks));
				} else {
					dateFrom = storedDateFrom;
					if (isDebugEnabled) {
						LOGGER.debug(String.format(
								"[%s][use storedDateFrom]diffMinutes=%s > currentMaxAge=%sm and countHistoryTasks=%s <= maxHistoryTasks=%s",
								method, diffMinutes, currentMaxAge, countHistoryTasks, maxHistoryTasks));
					}
				}
			} else {
				...
				if (isDebugEnabled) {
					LOGGER.debug(String.format(
							"[%s][use storedDateFrom]diffMinutes=%s(dateTo=%s-dateFrom=%s) <= currentMaxAge=%sm",
							method, diffMinutes, dateToAsString, ReportUtil.getDateAsString(dateFrom), currentMaxAge));
				}
			}
			// dateFrom = storedDateFrom;
		}
		if (dateFrom == null) {
			...
			if (isDebugEnabled) {
				LOGGER.debug(String.format("[%s][set dateFrom from null]dateFrom=%s (%s-%s)", method,
						ReportUtil.getDateAsString(dateFrom), dateToAsString, currentMaxAge));
			}
		}
		return dateFrom;
	}
	...
	private void pluginOnProcess(DBItemReportTrigger trigger, DBItemReportExecution execution,
			boolean reduceList4Notifications) {
		...
		if (trigger == null) {
			try {
				...
				if (isDebugEnabled) {
					LOGGER.debug(String.format("[%s][cannot get trigger for the given triggerId]%s[exception]%s",
							method, SOSString.toString(execution), e.toString()));
				}
				return;
			}
			if (trigger == null) {
				if (isDebugEnabled) {
					LOGGER.debug(String.format("[%s][not found trigger with the given triggerId]%s", method,
							SOSString.toString(execution)));
				}
				return;
			}
		}
		...
		if (reduceList4Notifications) {
			if (endedOrderTasks4notification.containsKey(execution.getHistoryId())) {
				...
				if (isDebugEnabled) {
					LOGGER.debug(String.format(
							"[pluginOnProcess][endedOrderTasks4notification][removed][task history id=%s]%s",
							execution.getHistoryId(), SOSString.toString(execution)));
				}
			}
		}
		if (isDebugEnabled) {
	...
	private void pluginOnProcess(DBItemReportTask task) {
		...
		if (task.getIsOrder()) {
			if (task.getEndTime() != null && !endedOrderTasks4notification.containsKey(task.getHistoryId())) {
				...
				if (isDebugEnabled) {
					LOGGER.debug(String.format(
							"[pluginOnProcess][endedOrderTasks4notification][added][task history id=%s]%s",
							task.getHistoryId(), SOSString.toString(task)));
				}
			}
			return;
		}
