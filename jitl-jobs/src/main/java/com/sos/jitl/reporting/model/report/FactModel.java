...
public class FactModel extends ReportingModel implements IReportingModel {
	...
	private void removeReportingEntries(ArrayList<String> schedulerIds, Date dateFrom, Date dateTo) throws Exception{
		...
		try{
			logger.info(String.format("%s: dateFrom = %s, dateTo = %s",
					method,
	...
	private void finishSynchronizing(Date dateTo) throws Exception {
		...
		try {
			logger.info(String.format("%s: dateTo = %s",
					method,
					ReportUtil.getDateAsString(dateTo)));
	...
	private void synchronize(Criteria criteria,String range, Date dateTo) throws Exception {
		...
		try {
			...
			while (sr.next()) {
				...
				if (step.getTaskId() == null && step.getTaskJobName() == null && step.getTaskCause() == null) {
					...
					logger.debug(String
							.format("%s: %s) task object is null. jobChain = %s, order = %s, step = %s, taskId = %s ",
									method, countTotal, step.getOrderJobChain(),
				...
				try{
					...
					else 
					{
						...
						DBItemReportTrigger rt = getDbLayer().createReportTrigger(
							step.getOrderSchedulerId(), step.getOrderHistoryId(),
							step.getOrderId(), null, step.getOrderJobChain(),
					...
					DBItemReportExecution re = createReportExecution(step.getOrderSchedulerId(),
						step.getOrderHistoryId(), triggerId,
						step.getStepStep(), step.getTaskJobName(),
						ReportUtil.getBasenameFromName(step.getTaskJobName()), null, step.getStepStartTime(),
						step.getStepEndTime(), 
						step.getStepState(),
						step.getTaskCause(), 
						step.isStepError(),
				...
				if(countTotal % options.log_info_step.value() == 0){
					logger.info(String.format("%s: %s history steps processed ...",
							method,
			...
			if(range.equals("uncompleted")){
				...
				logger.info(String.format("%s: total = %s, triggers = %s, executions = %s of %s, skip = %s ",
						method,
						counterSynchronizeOld.getTotal(),
				...
				logger.info(String.format("%s: total history steps = %s, triggers = %s, executions = %s of %s, skip = %s ",
						method,
						counterSynchronizeNew.getTotal(),
	...
	private void logSummary(Date dateFrom, Date dateTo,DateTime start) throws Exception {
		...
		logger.info(String.format("%s: removed entries (%s to %s): triggers = %s, executions = %s",
				method,
				from,
				to,
		...
		logger.info(String.format("%s: removed results: results = %s, execution dates = %s",
				method,
		...
		logger.info(String.format("%s: synchronized new entries (%s to %s): total history steps = %s, triggers = %s, executions = %s of %s, skip = %s ",
				method,
				from,
				to,
				counterSynchronizeNew.getTotal(),
	...
	private Date getReportingDateFrom(Date dateTo) throws Exception {
		...
		Date dateFrom	  = SOSString.isEmpty(schedulerVariable.getTextValue()) ? null : ReportUtil
				.getDateFromString(schedulerVariable.getTextValue());
				
		logger.debug(String.format("%s: currentMaxAge = %s, storedMaxAge = %s, dateFrom = %s",
				method,
				currentMaxAge,
				storedMaxAge,
				ReportUtil.getDateAsString(dateFrom)));
		...
		if(dateFrom != null){
			...
			if(options.force_max_history_age.value()){
				...
				if (currentMaxAge > 0) {
					...
					logger.debug(String.format("%s: diffMinutes = %s, currentMaxAge = %s, dateFrom = %s",
							method,
							diffMinutes,
							currentMaxAge,
							ReportUtil.getDateAsString(dateFrom)));
		...
		if (dateFrom == null && currentMaxAge > 0) {
			...
			logger.debug(String.format("%s: dateTo = %s - currentMaxAge = %s, dateFrom = %s",
					method,
					ReportUtil.getDateAsString(dateTo),
					currentMaxAge,
					ReportUtil.getDateAsString(dateFrom)));
		...
		logger.info(String.format("%s: dateFrom = %s (storedDateFrom = %s, max_history_age = %s (%s minutes), storedMaxAge = %s minutes)",
				method,
				ReportUtil.getDateAsString(dateFrom),
				schedulerVariable.getTextValue(),
				options.max_history_age.Value(),
				currentMaxAge,
				storedMaxAge
				));
		return dateFrom;
