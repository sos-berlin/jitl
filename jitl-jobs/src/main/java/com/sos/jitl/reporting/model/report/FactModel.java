...
public class FactModel extends ReportingModel implements IReportingModel {
    ...
    private void removeReportingEntries(ArrayList<String> schedulerIds, Date dateFrom, Date dateTo) throws Exception {
        ...
        try {
            logger.info(String.format("%s: dateFrom = %s, dateTo = %s", method, ReportUtil.getDateAsString(dateFrom), ReportUtil.getDateAsString(dateTo)));
        ...
        try {
            ...
            while (sr.next()) {
                ...
                if (step.getOrderHistoryId() == null && step.getOrderId() == null && step.getOrderStartTime() == null) {
                    countSkip++;
                    logger.debug(String.format("%s: %s) order object is null. step = %s, historyId = %s ", method, countTotal, step.getStepState(), step.getStepHistoryId()));

                    continue;
                }
                if (step.getTaskId() == null && step.getTaskJobName() == null && step.getTaskCause() == null) {
                    countSkip++;
                    logger.debug(String.format("%s: %s) task object is null. jobChain = %s, order = %s, step = %s, taskId = %s ", method, countTotal, step.getOrderJobChain(), step.getOrderId(), step.getStepState(), step.getStepTaskId()));
                    continue;
                }

                logger.debug(String.format("%s: %s) schedulerId = %s, orderHistoryId = %s, jobChain = %s, order id = %s, step = %s, step state = %s", method, countTotal, step.getOrderSchedulerId(), step.getOrderHistoryId(), step.getOrderJobChain(), step.getOrderId(), step.getStepStep(), step.getStepState()));
                ...
                try {
                    ...
                    if (inserted.containsKey(step.getOrderHistoryId())) {
                        ...
                        DBItemReportTrigger rt = getDbLayer().createReportTrigger(step.getOrderSchedulerId(), step.getOrderHistoryId(), step.getOrderId(), null, step.getOrderJobChain(), ReportUtil.getBasenameFromName(step.getOrderJobChain()), null, step.getOrderState(), step.getOrderStateText(), step.getOrderStartTime(), step.getOrderEndTime(), syncCompleted);
                    ...
                    DBItemReportExecution re = createReportExecution(step.getOrderSchedulerId(), step.getOrderHistoryId(), triggerId, step.getStepStep(), step.getTaskJobName(), ReportUtil.getBasenameFromName(step.getTaskJobName()), null, step.getStepStartTime(), step.getStepEndTime(), step.getStepState(), step.getTaskCause(), step.isStepError(), step.getStepErrorCode(), step.getStepErrorText());
            ...
            if (range.equals("uncompleted")) {
                ...
                logger.info(String.format("%s: total = %s, triggers = %s, executions = %s of %s, skip = %s ", method, counterSynchronizeOld.getTotal(), counterSynchronizeOld.getTriggers(), counterSynchronizeOld.getExecutionsBatch(), counterSynchronizeOld.getExecutions(), counterSynchronizeOld.getSkip()));
                ...
                logger.info(String.format("%s: total history steps = %s, triggers = %s, executions = %s of %s, skip = %s ", method, counterSynchronizeNew.getTotal(), counterSynchronizeNew.getTriggers(), counterSynchronizeNew.getExecutionsBatch(), counterSynchronizeNew.getExecutions(), counterSynchronizeNew.getSkip()));
    ...
    private void logSummary(Date dateFrom, Date dateTo, DateTime start) throws Exception {
        ...
        logger.info(String.format("%s: removed entries (%s to %s): triggers = %s, executions = %s", method, from, to, counterRemove.getTriggers(), counterRemove.getExecutions()));

        logger.info(String.format("%s: removed results: results = %s, execution dates = %s", method, counterRemove.getTriggerResults(), counterRemove.getExecutionDates()));

        logger.info(String.format("%s: synchronized new entries (%s to %s): total history steps = %s, triggers = %s, executions = %s of %s, skip = %s ", method, from, to, counterSynchronizeNew.getTotal(), counterSynchronizeNew.getTriggers(), counterSynchronizeNew.getExecutionsBatch(), counterSynchronizeNew.getExecutions(), counterSynchronizeNew.getSkip()));

        logger.info(String.format("%s: synchronized old entries: total uncompleted triggers = %s, triggers = %s, executions = %s of %s, skip = %s ", method, counterSynchronizeOld.getTotal(), counterSynchronizeOld.getTriggers(), counterSynchronizeOld.getExecutionsBatch(), counterSynchronizeOld.getExecutions(), counterSynchronizeOld.getSkip()));
    ...
    private Date getReportingDateFrom(Date dateTo) throws Exception {
        ...
        logger.debug(String.format("%s: currentMaxAge = %s, storedMaxAge = %s, dateFrom = %s", method, currentMaxAge, storedMaxAge, ReportUtil.getDateAsString(dateFrom)));
        ...
        if (dateFrom != null) {
            ...
            if (options.force_max_history_age.value()) {
                ...
                if (currentMaxAge > 0) {
                    ...
                    logger.debug(String.format("%s: diffMinutes = %s, currentMaxAge = %s, dateFrom = %s", method, diffMinutes, currentMaxAge, ReportUtil.getDateAsString(dateFrom)));
        ...
        if (dateFrom == null && currentMaxAge > 0) {
            ...
            logger.debug(String.format("%s: dateTo = %s - currentMaxAge = %s, dateFrom = %s", method, ReportUtil.getDateAsString(dateTo), currentMaxAge, ReportUtil.getDateAsString(dateFrom)));
        }

        logger.info(String.format("%s: dateFrom = %s (storedDateFrom = %s, max_history_age = %s (%s minutes), storedMaxAge = %s minutes)", method, ReportUtil.getDateAsString(dateFrom), schedulerVariable.getTextValue(), options.max_history_age.Value(), currentMaxAge, storedMaxAge));
        return dateFrom;
    }
    ...
    private DBItemReportExecution createReportExecution(String schedulerId, Long historyId, Long triggerId, Long step, String name, String basename,
            String title, Date startTime, Date endTime, String state, String cause, boolean error, String errorCode, String errorText) throws Exception {
