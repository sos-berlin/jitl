...
private void readMailOrders() throws RuntimeException, Exception{
      ...
      if (mailOrders.size() > 0) logger.info(mailOrders.size() + " mail files found");
  }
  ...
  private void sendMessage(File messageFile,int curDeliveryCounter) throws Exception{
      try { // to send
          ...
          if (jobSchedulerDequeueMailJobOptions.log_only.value()) {
              logger.info("mail was processed from file [" + messageFile.getAbsolutePath() + "] to: " + sosMail.getRecipientsAsString() + " into: " + mailFile.getAbsolutePath());
          } else {
