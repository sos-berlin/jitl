...
public class JobSchedulerDequeueMailJobOptionsSuperClass extends JSOptionsClass {
    ...
    /** \var db : This setting states that a database is used and that in order
     * to update their shipping state, mails are to be sought in a table. */
    ...
    /** \var failed_prefix : prefix for failed mail files prefix for failed mail
     * files */
    @JSOptionDefinition(name = "failed_prefix", description = "prefix for failed mail files", key = "failed_prefix", type = "SOSOptionString", mandatory = false)
     ...
     * 
     * \details prefix for failed mail files
     *
    ...
    /** \var file : This parameter provides the name of the file containing a
     * mail to be dequeued. The path is not specified with the filename but in
     * the queue_directory job parameter. */
    ...
    /** \var max_delivery : This parameter specifies the maximum number of
     * attempts to be made to send an email. If an email is sent then an
     * X-Header named X-SOSMail-delivery-counter with the value of the current
     * number of trials is created. If the value of this parameter is 0, then an
     * infinite number of attempts may be made to send a mail. For other values
     * the shipment will be cancelled once this number of attempts has been
     * reached. In this case, the mail will be stored in the dequeue directory
     * in a file with the prefix failed. . */
    ...
    /** \var queue_directory : This parameter contains the name of the directory
     * in which mails have been stored. If this value is left blank then the job
     * will use the dequeueing directory that was configured for the
     * JobScheduler and which is returned by the API Mail.dequeue() method. */
    ...
    /** \var queue_pattern : pattern for filenames of enqueued mails pattern for
     * filenames of enqueued mails */
    @JSOptionDefinition(name = "queue_pattern", description = "pattern for filenames of enqueued mails", key = "queue_pattern", type = "SOSOptionString", mandatory = false)
     ...
     * 
     * \details pattern for filenames of enqueued mails
     *
    ...
    /** \var queue_prefix : If an email cannot be sent due to mail server
     * problems, then it will be stored as a file. This prefix is then used in
     * the file name. */
    ...
    /** \var queue_prefix_spec : This parameter contains a regular expression to
     * specify the files that should be dequeued. The parameter is ignored if
     * this job is triggered by an order. */
