...
public class CSV2CSVModel {
				...
				if(printAllFields){
					...
					for(int index : headerIndexes){
						...
						try{
							...
							throw new Exception(String.format("[param \"fields\" = %s] not found field index = %s",
		...
		try{
			...
			if(headerMap == null){
				if(!hasNumericalFields){
					throw new Exception(String.format("param \"fields\" contains non-numeric values %s",
							options.fields.Value()));
				}
	...
	private CSVParser getCSVParser(FileReader reader) throws Exception{
		Character inputFileDelimeter = SOSString.isEmpty(options.input_file_delimiter.Value()) ? '\0' : options.input_file_delimiter.Value().charAt(0);
		Character inputFileQuoteCharacter = SOSString.isEmpty(options.input_file_quote_character.Value()) ? null : options.input_file_quote_character.Value().charAt(0);
		Character inputFileEscapeCharacter = SOSString.isEmpty(options.input_file_escape_character.Value()) ? null : options.input_file_escape_character.Value().charAt(0);
		
		CSVFormat formatReader = CSVFormat.newFormat(inputFileDelimeter)
				.withRecordSeparator(options.input_file_record_separator.Value())  
			    .withCommentMarker('#')
	            .withIgnoreEmptyLines(false)
	            .withQuote(inputFileQuoteCharacter)
	            .withQuoteMode(QuoteMode.ALL)
	            .withEscape(inputFileEscapeCharacter);
	   if(!hasNumericalFields){
	...
	private CSVPrinter getCSVPrinter(FileWriter writer) throws Exception{
		...
		CSVFormat formatWriter = CSVFormat.newFormat(delimeter)
