...
public class JobSchedulerSynchronizeJobChainsJSAdapterClass extends JobSchedulerJobAdapter {
    ...
    private void doProcessing() throws Exception {
        ...
        if (!objO.ignore_stopped_jobchains.isDirty()) {
            ...
            if (spooler.var(objO.ignore_stopped_jobchains.getShortKey()) != null && !spooler.var(objO.ignore_stopped_jobchains.getShortKey()).trim().isEmpty()) {
        ...
        if (objR.syncNodeContainer.isReleased()) {
            while (!objR.syncNodeContainer.eof()) {
                ...
                for (SyncNodeWaitingOrder objWaitingOrder : lstWaitingOrders) {
                    ...
                    String strJSCommand = "";
                    if (objO.setback_type.Value().equalsIgnoreCase(SYNC_METHOD_SETBACK)) {
                        strJSCommand = String.format("<modify_order job_chain='%s' order='%s' setback='no'>" + "<params><param name='scheduler_sync_ready' "
                                + "value='true'></param></params></modify_order>", objSyncNode.getSyncNodeJobchainPath(), objWaitingOrder.getId());
                    } else {
