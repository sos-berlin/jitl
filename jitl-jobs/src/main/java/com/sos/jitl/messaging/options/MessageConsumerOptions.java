...
public class MessageConsumerOptions extends MessageOptionSuperClass {
    ...
    @JSOptionDefinition(name = "targetJobChainName", description = "the name of the target JobChain(s) to receive the created order, if more than one JobChain should receive the order"
            + " the names of the JobChains can be separated with the [paramKeyValueDelimiter]", key = "targetJobChainName", type = "SOSOptionString", mandatory = false)
    public SOSOptionString targetJobChainName = new SOSOptionString(this, "targetJobChainName", "the name of the target JobChain to receive the created order", "", "", false);
    ...
    @JSOptionDefinition(name = "lastReceiver", description = "this parameter determines if this consumer is the last to receive the message", key = "lastReceiver", type = "SOSOptionBoolean", mandatory = false)
    public SOSOptionBoolean lastReceiver = new SOSOptionBoolean(this, "lastReceiver", "this parameter determines if this consumer is the last to receive the message", "false", "false", false);
    ...
    public void setLastReceiver(Boolean lastReceiver) {
        this.lastReceiver = new SOSOptionBoolean(this, "lastReceiver", "this parameter determines if this consumer is the last to receive the message", lastReceiver.toString(), lastReceiver.toString(), false);
    }
