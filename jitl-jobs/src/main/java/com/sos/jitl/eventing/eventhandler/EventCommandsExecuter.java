...
public class EventCommandsExecuter {
	...
	public EventCommandsExecuter(Collection<Object> eventHandlerResultedCommands,
			List<SchedulerEventDBItem> listOfEvents, String hostName, Integer httpPort, Integer socketTimeout) {
		super();
		...
		try {
			...
			while (eventHandlerResultedCommandsIterator.hasNext()) {
				...
				for (int i = 0; i < commands.getLength(); i++) {
					...
					try {
						LOGGER.debug(".. connecting to JobScheduler " + schedulerCommand.getHost() + ":"
						...
						for (int k = 0; k < commandElements.getLength(); k++) {
							...
							LOGGER.info(".. sending command to remote JobScheduler [" + commandHost + ":" + commandPort
									+ "]: " + commandRequest);
	...
	private void getParametersFromEvents() throws Exception {
		...
		for (SchedulerEventDBItem event : listOfEvents) {
			...
			if (event.getParameters() != null) {
				Element nodeParameters = DocumentBuilderFactory.newInstance().newDocumentBuilder()
						.parse(new ByteArrayInputStream(event.getParameters().getBytes())).getDocumentElement();
				...
				if (parameters != null && parameters.getLength() > 0) {
					for (int ii = 0; ii < parameters.getLength(); ii++) {
						...
						parameterSubstitutor.addKey(event.getEventClass() + "." + event.getEventId() + "." + paramName,
								paramValue);
						...
						LOGGER.debug(
								event.getEventClass() + "." + event.getEventId() + "." + paramName + "=" + paramValue);
					}
				}
			}
		}
	...
	private StringBuffer getFileContent(final File file) throws Exception {
		...
		try {
			...
			throw new Exception(
					"error occurred reading content of file [" + file.getCanonicalPath() + "]: " + e.getMessage(), e);
		} finally {
			if (in != null) {
				in.close();
				in = null;
