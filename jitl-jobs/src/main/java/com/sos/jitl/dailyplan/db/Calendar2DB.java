...
public class Calendar2DB {
	...
	public List<DailyPlanDBItem> getStartTimesFromScheduler(Date from, Date to, PlanFilter planFilter)
	...
	private void storeJobStreamStarters(FilterJobStreams filterJobStreams, String timezone) throws Exception {
		...
		DBLayerJobStreamsStarterJobs dbLayerJobStreamsStarterJobs = new DBLayerJobStreamsStarterJobs(
				dailyPlanDBLayer.getSession());
		...
		List<DBItemJobStreamStarter> listOfStarters = dbLayerJobStreamStarters
		...
		for (DBItemJobStreamStarter dbItemJobStreamStarter : listOfStarters) {
			...
			for (com.sos.joc.model.calendar.Period period : jsStarter.getJobStreamScheduler().getPlan().getPeriods()) {
				...
				for (DBItemJobStreamStarterJob dbItemJobStreamStarterJob : dbLayerJobStreamsStarterJobs
						.getJobStreamStarterJobsList(filterJobStreamStarterJobs, 0)) {
					boolean isNew;
					...
					if (period.getSingleStart() != null) {
						...
						LOGGER.debug("start jobstream: " + jsStarter.getItemJobStreamStarter().getJobStream() + " at "
								+ start);
					} else {
					...
					if (period.getSingleStart() != null) {
						...
						try {
							dailyPlanDBItem.setRepeatInterval(new BigInteger(period.getAbsoluteRepeat()),
									new BigInteger(period.getRepeat()));
						} catch (Exception e) {
						}
						LOGGER.debug("start jobstream: " + jsStarter.getItemJobStreamStarter().getJobStream() + " at "
								+ start);
					...
					if (mapOfJobStreams.get(dbItemJobStreamStarter.getJobStream()) != null) {
						dailyPlanDBItem.setJobStream(
								mapOfJobStreams.get(dbItemJobStreamStarter.getJobStream()).getJobStream());
					} else {
					...
					if (dailyPlanDBItem.getPlannedStart() != null) {
						dailyPlanDBItem
								.setExpectedEnd(new Date(dailyPlanDBItem.getPlannedStart().getTime() + duration));
					}
					...
					if (isNew) {
						...
						LOGGER.debug("Store daily plan job stream item:" + dailyPlanDBItem.getId() + " at "
								+ dailyPlanDBItem.getPlannedStartFormated());
					} else {
						try {
							dailyPlanDBLayer.getSession().update(dailyPlanDBItem);
							LOGGER.debug("Update daily plan job stream item:" + dailyPlanDBItem.getId() + " at "
									+ dailyPlanDBItem.getPlannedStartFormated());
	...
	public void addDailyplan2DBFilter(DailyPlanCalender2DBFilter dailyPlanCalender2DBFilter, Long instanceId)
			throws SOSHibernateException, ParseException {
		...
		if ((!"".equals(dailyPlanCalender2DBFilter.getForSchedule())
				&& (dailyPlanCalender2DBFilter.getForSchedule() != null))) {
			...
			if (instanceId != null) {
				String schedule;
				DBItemInventorySchedule dbItemInventorySchedule = dbLayerInventory
						.getSubstituteIfExists(dailyPlanCalender2DBFilter.getForSchedule(), instanceId);
				...
				List<DBItemInventoryJob> listOfUsedJobs = dbLayerInventory.getJobsReferencingSchedule(instanceId,
						schedule);
				for (DBItemInventoryJob dbItemInventoryJob : listOfUsedJobs) {
					...
					listOfDailyPlanCalender2DBFilter.put(dailyPlanCalender2DBFilter.getKey(),
							dailyPlanCalender2DBFilter);

				}
				List<DBItemInventoryOrder> listOfUsedOrders = dbLayerInventory.getOrdersReferencingSchedule(instanceId,
						schedule);
	...
	public void processDailyplan2DBFilter() throws Exception {
		...
		initSchedulerConnection();
		LOGGER.debug(String.format("processDailyplan2DBFilter: day_offset is %s SchedulerId is %s", this.dayOffset,
				this.schedulerId));
		if (from == null) {
			setFrom();
		}
		...
		try {
			...
			if (percentage < 90) {
				...
				for (Map.Entry<String, DailyPlanCalender2DBFilter> entry : listOfDailyPlanCalender2DBFilter
						.entrySet()) {
					...
					final long timeEnd = System.currentTimeMillis();
					LOGGER.debug(
							"Duration: " + dailyPlanCalender2DBFilter.getName() + ":" + (timeEnd - timeStart) + " ms");
			...
			if (spooler == null) {
				if (options.basicAuthorization.isDirty() && !options.basicAuthorization.getValue().isEmpty()) {
					schedulerObjectFactory = new SchedulerObjectFactory(options.getCommandUrl().getValue(),
							options.basicAuthorization.getValue());
				} else {
					schedulerObjectFactory = new SchedulerObjectFactory(options.getCommandUrl().getValue());
				}
			} else {
	...
	private Calendar getCalendar(Date start, Date before, boolean withTime) {
		...
		LOGGER.debug(String.format("... calculating plan for %s - %s ", jsCmdShowCalendar.getFrom(),
				jsCmdShowCalendar.getBefore()));
	...
	private void deleteItemsAfterTo(DailyPlanCalender2DBFilter dailyPlanCalender2DBFilter)
	...
	private Long getDuration(DBLayerReporting dbLayerReporting, String job, Order order) throws SOSHibernateException {
		...
		if (duration == null) {
			...
			if (order == null) {
				...
				duration = dbLayerReporting.getOrderEstimatedDuration(order.getJobChain(), order.getId(),
						DEFAULT_LIMIT);
			}
			...
			for (Object calendarObject : dailyPlanCalendarItem.getCalendar().getAtOrPeriod()) {
				...
				if (calendarObject instanceof Period) {
					...
					boolean handleEntry = (listOfPlanEntries.get(getUniqueKey(dailyPlanEntry)) == null
							&& !isBeforeToday(dailyPlanEntry.getPlannedStart()) && (dailyPlanCalender2DBFilter == null
	...
	private List<DailyPlanDBItem> getCalendarFromJobScheduler(PlanFilter planFilter)
			throws ParseException, SOSHibernateException {
		...
		for (DailyPlanCalendarItem dailyPlanCalendarItem : listOfCalendars) {
			...
			from = dailyPlanCalendarItem.getFrom();
			Date utcFrom = UtcTimeHelper.convertTimeZonesToDate(fromTimeZoneString, toTimeZoneString,
					new DateTime(from).withZone(fromZone));

			to = dailyPlanCalendarItem.getTo();
			LOGGER.debug(String.format("Starttimes from Calendar: from=%s  to=%s", dailyPlanCalendarItem.getFrom(),
					dailyPlanCalendarItem.getTo()));
				...
				if (calendarObject instanceof Period) {
					...
					if (job != null || order != null) {
						if (singleStart != null) {
							...
							if (dailyPlanDBItem.getPlannedStart() != null
									&& dailyPlanDBItem.getPlannedStart().compareTo(utcFrom) < 0
