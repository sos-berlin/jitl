...
public class StoreResultsJob extends JSJobUtilitiesClass<StoreResultsJobOptions> {
    ...
    public void init() throws Exception {
        try {
            factory = new SOSHibernateFactory(getOptions().scheduler_notification_hibernate_configuration_file.getValue());
            factory.setAutoCommit(getOptions().scheduler_notification_connection_autocommit.value());
            factory.setTransactionIsolation(getOptions().scheduler_notification_connection_transaction_isolation.value());
