...
public class JobSchedulerCheckRunHistoryOptionsJUnitTest extends JSToolBox {
	...
	private static Logger logger = Logger
			.getLogger(JobSchedulerCheckRunHistoryOptionsJUnitTest.class);
	...
	public void testJobChainName() { // JSJobChainName
		...
		assertEquals("The name of a job chain.",
				objOptions.jobChainName.Value(), "++----++");
	...
	public void testmail_bcc() { // JSOptionMailOptions
		...
		assertEquals(
				"Email blind carbon copy address of the recipient, see ./c",
				objOptions.mail_bcc.Value(), "++----++");
	...
	public void testmail_cc() { // JSOptionMailOptions
		...
		assertEquals(
				"Email carbon copy address of the recipient, see ./config/",
				objOptions.mail_cc.Value(), "++----++");
	...
	public void testmail_to() { // JSOptionMailOptions
		...
		assertEquals(
				"Email address of the recipient, see ./config/factory.ini,",
				objOptions.mail_to.Value(), "++----++");
	...
	public void testmessage() { // SOSOptionString
		...
		assertEquals("Text in the email subject and in the log.",
				objOptions.message.Value(), "++----++");
	...
	public void testoperation() { // SOSOptionStringValueList
		...
		assertEquals("Operation to be executed", objOptions.query.Value(),
				"++late++");
	...
	public void testOrderId() { // JSOrderId
		...
		assertEquals("The name or the identification of an order.",
				objOptions.orderId.Value(), "++----++");

	}

	/**
	 * \brief testend_time : The start time from which the parametrisized job
	 * is check
	 * 
	...
	public void testend_time() { // SOSOptionString
		...
		assertEquals(
				"The end time from which the parametrisized job is check",
				objOptions.end_time.Value(), "++0,00:00:00++");
	...
	public void teststart_time() { // SOSOptionString
		...
		assertEquals(
				"The start time from which the parametrisized job is check",
				objOptions.start_time.Value(), "++0,00:00:00++");
	...
	public void testfailOnQueryResultFalse() { // SOSOptionBoolean
 		assertEquals(
				"testfailOnQueryResultFalse",
				objOptions.failOnQueryResultFalse.value(), true);
