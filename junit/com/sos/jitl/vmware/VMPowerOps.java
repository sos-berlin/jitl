...
public class VMPowerOps {
	...
	private static void getConnectionParameters(final String[] args) throws IllegalArgumentException {
		int ai = 0;
		String param = "";
		String val = "";
		while (ai < args.length) {
			...
			if (param.equalsIgnoreCase("--help")) {
				...
				if (param.equalsIgnoreCase("--url") && !val.startsWith("--") && !val.isEmpty()) {
					...
					if (param.equalsIgnoreCase("--username") && !val.startsWith("--") && !val.isEmpty()) {
						userName = val;
					}
					else
						if (param.equalsIgnoreCase("--password") && !val.startsWith("--") && !val.isEmpty()) {
							password = val;
						}
			val = "";
			ai += 2;
	...
	private static void getInputParameters(final String[] args) {
		int ai = 0;
		String param = "";
		String val = "";
		while (ai < args.length) {
			...
			if (param.equalsIgnoreCase("--vmname") && !val.startsWith("--") && !val.isEmpty()) {
				...
				if (param.equalsIgnoreCase("--operation") && !val.startsWith("--") && !val.isEmpty()) {
					operation = val;
				}
				else
					if (param.equalsIgnoreCase("--datacentername") && !val.startsWith("--") && !val.isEmpty()) {
						datacenter = val;
					}
					else
						if (param.equalsIgnoreCase("--guestid") && !val.startsWith("--") && !val.isEmpty()) {
							guestId = val;
						}
						else
							if (param.equalsIgnoreCase("--hostname") && !val.startsWith("--") && !val.isEmpty()) {
								host = val;
							}
			val = "";
			ai += 2;
	...
	private static void validate() throws IllegalArgumentException {
		if (!operation.equalsIgnoreCase("poweron") && !operation.equalsIgnoreCase("poweroff") && !operation.equalsIgnoreCase("reset")
				&& !operation.equalsIgnoreCase("standby") && !operation.equalsIgnoreCase("shutdown") && !operation.equalsIgnoreCase("reboot")
				&& !operation.equalsIgnoreCase("suspend")) {

			System.out.println("Invalid Operation name ' " + operation + "' valid operations are poweron, standby,"
	 ...
	 */
	private static Map<String, ManagedObjectReference> getMOREFsInContainerByType(final ManagedObjectReference folder, final String morefType)
	 ...
	 * @throws RuntimeFaultFaultMsg
	 */
	private static Map<ManagedObjectReference, Map<String, Object>> getEntityProps(final List<ManagedObjectReference> entityMors, final String[] props)
			throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg {
	 ...
	 * @throws InvalidPropertyFaultMsg
	 */
	private static boolean getTaskResultAfterDone(final ManagedObjectReference task) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg,
	 ...
	 * @throws InvalidCollectorVersionFaultMsg
	 */
	private static Object[] waitForValues(final ManagedObjectReference objmor, final String[] filterProps, final String[] endWaitProps,
			final Object[][] expectedVals) throws InvalidPropertyFaultMsg, RuntimeFaultFaultMsg, InvalidCollectorVersionFaultMsg {
	...
	private static void updateValues(final String[] props, final Object[] vals, final PropertyChange propchg) {
	...
	private static Map<String, ManagedObjectReference> getVms() throws Exception {
		...
		if (datacenter != null) {
			ManagedObjectReference dcMoref = getMOREFsInContainerByType(container, "Datacenter").get(datacenter);
			if (dcMoref == null) {
				System.out.println("No datacenter by the name " + datacenter + " found!");
				return vmList;
			}
			container = dcMoref;
		}
		...
		if (host != null) {
			ManagedObjectReference hostMoref = getMOREFsInContainerByType(container, "HostSystem").get(host);
			if (hostMoref == null) {
		...
		Map<String, ManagedObjectReference> vms = getMOREFsInContainerByType(container, "VirtualMachine");
		...
		if (guestId != null) {
			Map<ManagedObjectReference, Map<String, Object>> vmListProp = getEntityProps(new ArrayList<ManagedObjectReference>(vms.values()), new String[] {
					"summary.config.guestId", "name" });
			for (ManagedObjectReference vmRef : vmListProp.keySet()) {
				if (guestId.equalsIgnoreCase((String) vmListProp.get(vmRef).get("summary.config.guestId"))) {
	...
	private static void runOperation() throws Exception {
		...
		if (vmMap == null || vmMap.isEmpty()) {
			...
			if (operation.equalsIgnoreCase("poweron")) {
				...
				if (operation.equalsIgnoreCase("poweroff")) {
					powerOffVM(vmMap);
				}
				else
						...
						if (operation.equalsIgnoreCase("suspend")) {
							...
							if (operation.equalsIgnoreCase("reboot")) {
								...
								if (operation.equalsIgnoreCase("shutdown")) {
									...
									if (operation.equalsIgnoreCase("standby")) {
										standbyVM(vmMap);
									}
		}
	}

	private static void powerOnVM(final Map<String, ManagedObjectReference> vmMap) {
	...
	private static void powerOffVM(final Map<String, ManagedObjectReference> vmMap) {
		for (String vmname : vmMap.keySet()) {
			ManagedObjectReference vmMor = vmMap.get(vmname);
			try {
	...
	private static void resetVM(final Map<String, ManagedObjectReference> vmMap) {
		for (String vmname : vmMap.keySet()) {
			ManagedObjectReference vmMor = vmMap.get(vmname);
			try {
				System.out.println("Reseting virtual machine : " + vmname + "[" + vmMor.getValue() + "]");
				ManagedObjectReference taskmor = vimPort.resetVMTask(vmMor);
				if (getTaskResultAfterDone(taskmor)) {
					System.out.println(vmname + "[" + vmMor.getValue() + "] reset successfully");
				}
			}
			catch (Exception e) {
				System.out.println("Unable to reset vm : " + vmname + "[" + vmMor.getValue() + "]");
				System.err.println("Reason :" + e.getLocalizedMessage());
			}
		}
	}

	private static void suspendVM(final Map<String, ManagedObjectReference> vmMap) {
		for (String vmname : vmMap.keySet()) {
			ManagedObjectReference vmMor = vmMap.get(vmname);
			try {
				System.out.println("Suspending virtual machine : " + vmname + "[" + vmMor.getValue() + "]");
				ManagedObjectReference taskmor = vimPort.suspendVMTask(vmMor);
				if (getTaskResultAfterDone(taskmor)) {
					System.out.println(vmname + "[" + vmMor.getValue() + "] suspended successfully");
				}
			}
			catch (Exception e) {
				System.out.println("Unable to suspend vm : " + vmname + "[" + vmMor.getValue() + "]");
				System.err.println("Reason :" + e.getLocalizedMessage());
			}
		}
	}

	private static void rebootVM(final Map<String, ManagedObjectReference> vmMap) {
		for (String vmname : vmMap.keySet()) {
			ManagedObjectReference vmMor = vmMap.get(vmname);
			try {
				System.out.println("Rebooting guest os in virtual machine : " + vmname + "[" + vmMor.getValue() + "]");
				vimPort.rebootGuest(vmMor);
				System.out.println("Guest os in vm : " + vmname + "[" + vmMor.getValue() + "]" + " rebooted");
			}
			catch (Exception e) {
				System.out.println("Unable to reboot guest os in vm : " + vmname + "[" + vmMor.getValue() + "]");
				System.err.println("Reason :" + e.getLocalizedMessage());
			}
		}
	}

	private static void shutdownVM(final Map<String, ManagedObjectReference> vmMap) {
		for (String vmname : vmMap.keySet()) {
			ManagedObjectReference vmMor = vmMap.get(vmname);
			try {
				System.out.println("Shutting down guest os in virtual machine : " + vmname + "[" + vmMor.getValue() + "]");
				vimPort.shutdownGuest(vmMor);
				System.out.println("Guest os in vm : " + vmname + "[" + vmMor.getValue() + "]" + " shutdown");
			}
			catch (Exception e) {
				System.out.println("Unable to shutdown guest os in vm : " + vmname + "[" + vmMor.getValue() + "]");
				System.err.println("Reason :" + e.getLocalizedMessage());
			}
		}
	}

	private static void standbyVM(final Map<String, ManagedObjectReference> vmMap) {
		for (String vmname : vmMap.keySet()) {
			ManagedObjectReference vmMor = vmMap.get(vmname);
			try {
				System.out.println("Putting the guest os in virtual machine : " + vmname + "[" + vmMor.getValue() + "] in standby mode");
				vimPort.standbyGuest(vmMor);
				System.out.println("Guest os in vm : " + vmname + "[" + vmMor.getValue() + "]" + " in standby mode");
			}
			catch (Exception e) {
				System.out.println("Unable to put the guest os in vm : " + vmname + "[" + vmMor.getValue() + "] to standby mode");
				System.err.println("Reason :" + e.getLocalizedMessage());
			}
		}
	}
	...
	private static void printUsage() {
		System.out.println("This sample Performs power ops(on|off etc.) " + "operations on the virtual machines");
		System.out.println("\nParameters:");
